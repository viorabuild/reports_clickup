...
3. Отправляет промпт в LM Studio с моделью `openai/gpt-oss-20b`, получая скорость, качество и короткое пояснение.
...
- Локально запущенный LM Studio HTTP-сервер (по умолчанию `http://127.0.0.1:1234`) с моделью `openai/gpt-oss-20b`
...
| `LM_STUDIO_MODEL` | Имя модели в LM Studio. По умолчанию `openai/gpt-oss-20b`. |
